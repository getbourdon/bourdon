"""Agent-agnostic turn-scoped recognition compiler.

This module builds a tiny, ranked recognition brief for one agent turn. It is
deliberately independent of any native memory stage succeeding: native memory
health is reported as a routing signal, but the brief is compiled from stronger
surfaces such as cwd/repo identity, local session metadata, and the L6
federation library.

Everything here is agent-agnostic. The pieces that differ between agents
(Codex, Claude Code, ...) are supplied by a :class:`SessionSource` — the seam
that this compiler is parameterized over. See ``core/codex_turn_compiler.py``
and ``core/claude_turn_compiler.py`` for the two concrete callers, and
``docs/turn-compiler-architecture.md`` for the design.
"""
...
class SessionSource(Protocol):
    """The agent-specific seam of the turn compiler.

    A ``SessionSource`` supplies the identity labels and the read-only
    native-surface probes that differ between agents. Everything else in this
    module is shared. Implementations are plain classes (no inheritance
    required); see ``CodexSessionSource`` and ``ClaudeSessionSource``.
    """

    #: Federation agent id, e.g. ``"codex"`` / ``"claude-code"``. Used to decide
    #: whether an L5 entity/session is own-agent or cross-agent (l6_federation).
    agent_id: str
    #: Human display name used in routing prose, e.g. ``"Codex"`` / ``"Claude"``.
    agent_display: str
    #: Output schema version, e.g. ``"codex-turn-brief/v1"``.
    schema_version: str
    #: Source label for own-agent L5 items, e.g. ``"codex_l5"`` / ``"claude_l5"``.
    l5_source_label: str
    #: Health dict/field key, e.g. ``"native_stage1"`` / ``"native_memory"``.
    native_health_key: str
    #: Health noun used in rendered prose, e.g. ``"native Stage 1"``.
    native_health_noun: str
    #: Noun for a local native record in scoring reasons, e.g. ``"Codex thread"``.
    local_record_noun: str
    #: Diagnostic list of exhausted/avoided recognition paths.
...
@dataclass
class BriefHealth:
    """Native-memory health signal.

    Stored generically (``value`` + ``key``) so each agent serializes it under
    the right field name. The ``native_stage1`` property preserves the original
    Codex attribute access.
    """

    value: str
    key: str = "native_stage1"
    strategy: str = STRATEGY

    @property
    def native_stage1(self) -> str:
        return self.value
...
def compile_turn(
    ...
    *,
    ...
    """Compile a turn-scoped recognition brief for the given ``source``.

    The function is read-only. It does not write native files, mutate the
    ...
    resolved_home = source.resolve_home(home)
    resolved_library = Path(library_path) if library_path else DEFAULT_LIBRARY_PATH

    state_report = source.inspect_native(resolved_home)
    native_health = source.classify_native(state_report)

    store = L6Store(resolved_library)
    manifest = store.build_recognition_manifest(access_level=access)
    candidates = _gather_candidates(prompt_text, manifest, source, resolved_home)
    scored = _score_candidates(candidates, prompt_text, cwd_path, repo, source.local_record_noun)
    ...
    routing = _routing_decision(
        ...
        repo,
        native_health,
        source.native_health_key,
        source.native_health_noun,
        source.agent_display,
    )
    trace = _recognition_trace(
        items, scored, repo, native_health, routing, source.native_health_key
    )

    explicit_text = _render_explicit_text(
        items, repo, native_health, source.native_health_noun, char_limit
    )
    ...
    diagnostics = _diagnostics(
        scored,
        source.native_diagnostics(state_report),
        delivery_mode,
        item_limit,
        char_limit,
        source.exhausted_paths,
    )
    ...
    return TurnBrief(
        ...
        cwd=cwd_text,
        repo=repo,
        health=BriefHealth(value=native_health, key=source.native_health_key),
        routing=routing,
        items=items,
...
def _gather_candidates(
    ...
    own_agent = source.agent_id
    own_label = source.l5_source_label
    for entity in manifest.get("known_entities") or []:
        ...
        source_label = own_label if source_agents == [own_agent] else "l6_federation"
        candidates.append(
            _Candidate(
    ...
    for session in manifest.get("recent_sessions") or []:
        ...
        source_label = own_label if agent == own_agent else "l6_federation"
        candidates.append(
            _Candidate(
                kind="session",
                name=name,
    ...
    candidates.extend(source.collect_local_records(home, limit=60))
...
def _score_candidates(
    ...
    for candidate in candidates:
        ...
        cwd_score, cwd_reason = _cwd_score(candidate, cwd, repo, local_record_noun)
...
def _passes_recognition_gate(
    ...
    if not repo_name:
        return False
    semantic_text = " ".join([candidate.name, candidate.summary, *candidate.project_focus]).lower()
...
def _meaningful_prompt_terms(prompt: str) -> list[str]:
    return [token for token in _tokens(prompt) if token not in _PROMPT_STOPWORDS and len(token) > 2]
...
def _cwd_score(
    ...
    if candidate.cwd and cwd:
        ...
        if _same_or_nested_path(candidate_path, cwd):
            ...
            reasons.append(f"cwd matched prior {local_record_noun}")
        elif repo.root and _same_or_nested_path(candidate_path, Path(repo.root)):
            score = max(score, 22.0)
            reasons.append(f"repo root matched prior {local_record_noun}")
...
def _penalty(candidate: _Candidate) -> float:
    ...
    if candidate.native_stage1_only:
        penalty -= 8.0
    if candidate.source.endswith("_l5") and candidate.date_text:
...
def _render_explicit_text(
    ...
    lines = [
        ...
        f"Strategy: turn-scoped compiler; {health_noun} is {health_value}.",
    ]
    if repo.name:
...
def _routing_decision(
    ...
    if not items:
        return {
            ...
            "suppressed_surfaces": [health_key, "memory_md", "fallback_file"],
            "next_action": "continue without recognition injection",
        }

    top_item = items[0]
    confidence = _confidence(top_item.score)
    surfaces = _recommended_surfaces(top_item, repo, health_value)
    suppressed = _suppressed_surfaces(health_value, confidence, health_key)
    return {
        "mode": "inject",
        "primary_surface": surfaces[0],
        "surfaces": surfaces,
        "confidence": confidence,
        "reason": _routing_reason(top_item, repo, health_value, health_noun, scored),
        "suppressed_surfaces": suppressed,
        "next_action": _routing_next_action(surfaces[0], agent_display),
    }
...
def _recommended_surfaces(
    ...
    if health_value == "available" and top_item.score >= 60:
        surfaces.append("native_memory_supporting")
    return surfaces


def _suppressed_surfaces(health_value: str, confidence: str, health_key: str) -> list[str]:
    suppressed: list[str] = []
    if health_value == "degraded":
        suppressed.append(f"{health_key}_primary")
    if confidence == "low":
...
def _routing_reason(
    ...
    if health_value == "degraded":
        pieces.append(f"{health_noun} is degraded, so active injection is preferred")
    return "; ".join(pieces)


def _routing_next_action(primary_surface: str, agent_display: str) -> str:
    if primary_surface == "explicit_pre_turn":
        return f"prepend delivery.explicit_text before the {agent_display} turn"
...
def _dominant_components(components: dict[str, float]) -> list[str]:
    positive = [
        (name, value) for name, value in components.items() if name != "penalty" and value > 0
    ]
...
def _diagnostics(
    ...
    diagnostics["exhausted_paths"] = exhausted_paths
    return diagnostics
