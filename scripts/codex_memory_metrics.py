...
The output is a graph-ready JSON/YAML snapshot intended for weekly tracking and
later association/pattern-recognition analysis. The script never reads
...
def _memory_files_snapshot(codex_home: Path | None) -> dict[str, Any]:
    memories_dir = codex_home / "memories" if codex_home else Path.home() / ".codex" / "memories"
    return {
        ...
        "bourdon_fallback_md": _file_snapshot(_default_codex_native_memory_path(codex_home)),
...
def _classify_stage1_errors(state_db_report: dict[str, Any]) -> dict[str, int]:
    ...
    for error in jobs.get("errors") or []:
        existing_class = error.get("error_class")
        if isinstance(existing_class, str) and existing_class:
            counter[existing_class] += 1
            continue
...
def _stage1_error_class(last_error: Any) -> str:
    text = str(last_error or "").lower()
    if "usage limit" in text:
        return "usage_limit"
    if "context window" in text or "ran out of room" in text:
...
def _redact_state_db_report(report: dict[str, Any]) -> dict[str, Any]:
    redacted = dict(report)
    jobs = dict(redacted.get("memory_stage1_jobs") or {})
    errors = []
    for error in jobs.get("errors") or []:
        ...
        errors.append(
            {
                ...
                "retry_remaining": error.get("retry_remaining"),
                "error_class": _stage1_error_class(error.get("last_error")),
            }
        )
    if jobs:
        jobs["errors"] = errors
        redacted["memory_stage1_jobs"] = jobs
    return redacted
...
def _derived_metrics(
    ...
    return {
        ...
        "native_accumulating": int(stage1_outputs.get("total") or 0) > 0
        and distilled_memory_items > 0,
    }
...
def build_snapshot(
    ...
    state_db_report = _redact_state_db_report(
        _inspect_codex_state_db(resolved_codex_home)
    )
