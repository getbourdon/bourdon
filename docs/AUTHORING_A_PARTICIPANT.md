# Authoring a Bourdon Participant
...
> **Canonical version of this guide** lives at `claude-brain/skills/bourdon-participant-authoring/SKILL.md` in the Bourdon authors' shared knowledge repo and auto-loads in their Claude Code sessions. This file is the public mirror for external contributors. Behavioral content is identical; if you spot drift, the claude-brain copy wins and a sync PR against this file is welcome.

**Canonical sources (read these first when you need detail beyond what's here):**

- [`spec/PARTICIPANT_CONTRACT.md`](../spec/PARTICIPANT_CONTRACT.md) — the formal contract, semantic requirements, error semantics, idempotency guarantees, testing requirements. Comprehensive. **This guide is operational; the spec doc is normative.**
- [`spec/L5_schema.json`](../spec/L5_schema.json) — JSON Schema all manifests must validate against
- [`participants/base.py`](../participants/base.py) — `BourdonParticipant` Protocol, dataclasses, helpers
- [`core/l5_io.py`](../core/l5_io.py) — atomic write functions
- [`participants/codex.py`](../participants/codex.py) — most complete reference impl. Includes credential redaction, fallback memory recognition, MCP server enrichment. **Read this when in doubt — copy patterns rather than re-inventing.**
- [`participants/cursor.py`](../participants/cursor.py) — clean SQLite-based reference impl
- [`cli/main.py`](../cli/main.py) (`_handle_cursor_export`, ~line 152) — clean subparser handler reference

## Step 0 — Pick the Participant Shape
...
| **External participant** (Python) | Agent stores native state on disk; we read it post-hoc (Claude Code, Codex, Cursor) | `bourdon/participants/<id>.py`, registered via `[project.entry-points."bourdon.participants"]` |
...
**Decision rule:** if the agent has a real plugin SDK and audience-side install pattern, ship a platform-native plugin even when an external participant would also work — it's where adoption lives. The external participant is a fallback when the agent has no plugin surface.

## Step 1 — Implement the Protocol (External Participant)

For external participants, satisfy `bourdon.participants.base.BourdonParticipant`:

```python
from participants.base import (
    BourdonParticipant, AgentInfo, Entity, Session, L5Manifest,
    AgentStore, HealthStatus, Visibility, VisibilityPolicy,
    ParticipantDiscoveryError, apply_visibility, filter_for_federation,
    CONTRACT_VERSION,  # "0.1"
)

class MyAgentParticipant:  # implicitly satisfies the Protocol
...
3. **Visibility filtering happens INSIDE the participant.** L6 trusts what you emit. Use `filter_for_federation(entities, policy)` from `participants.base` — don't roll your own filter.
4. **Errors:**
   - Native store missing → `ParticipantDiscoveryError`
   - Native store present but parse failed → `ParticipantExportError`
   - Native store newer/older than supported → `ParticipantVersionMismatchError`
   - Anything else → catch + convert to `HealthStatus.degraded` (never propagate)

## Step 2 — Apply Credential Redaction

**Reuse — do not re-implement.** Import the patterns from `participants/codex.py`:
...
If your agent has agent-specific credential patterns (e.g., a vendor-specific token prefix), extend the tuple in your participant module:
...
out_path = Path.home() / "agent-library" / "agents" / f"{participant.agent_id}.l5.yaml"
...
def _handle_<id>_export(args):
    participant = MyAgentParticipant(<args>)
...
my-agent = "participants.my_agent:MyAgentParticipant"   # NEW
```

The entry-point name is canonical — it MUST match `agent_id`. The CLI and the L6 server discover participants by iterating this group; no central registry.

## Step 6 — Test Suite (Mandatory Categories)

Per [`spec/PARTICIPANT_CONTRACT.md`](../spec/PARTICIPANT_CONTRACT.md) §Testing, every participant MUST ship four test categories. Reference impl: [`tests/test_codex_participant.py`](../tests/test_codex_participant.py) (372 lines). Layout:
...
└── test_<agent_id>_participant.py       # the test file
```
...
|---|---|---|
| **Discovery** | `discover()` raises `ParticipantDiscoveryError` on missing store; returns `AgentStore` on present store | Required by Protocol; smallest surface |
...
| **Redaction** (when emitting native text) | A fixture with `password=secret123` produces `[redacted credential-like text]` in the manifest | Required when participant surfaces native string content |

The Codex participant test file also covers fallback memory recognition, MCP enrichment, and CLI handler smoke. Mirror those patterns where the agent has analogous surfaces.
...
When the agent has a plugin SDK (OpenClaw is the reference case), a Bourdon **plugin** in their idiom is the right shipping shape, not an external participant. The plugin must:
...
5. **Update `docs/agent-integration-status.md` with the plugin status** (e.g., "Status: native plugin published to clawhub. Reads agent memory via plugin-sdk; exposes L6 tools as agent tools."). The Bourdon repo doesn't get a Python participant for these — the plugin is the integration.
...
- **Don't ship a Python participant when the agent has a plugin SDK.** The plugin is where the install path is. The Python participant is the fallback for SDK-less agents.

## Verification before claiming "shipped"

A new participant is shipped when ALL of these are true:

- [ ] `pytest tests/test_<id>_participant.py` is green (4+ test categories)
- [ ] `python -c "from importlib.metadata import entry_points; print([ep.name for ep in entry_points(group='bourdon.participants')])"` lists the new agent
...
Every participant described above reads from a local store the agent owns (Cursor's SQLite, Codex's session_index.jsonl, Claude Code's claude-brain, Copilot/Cascade's convention file). That shape doesn't fit **cloud-only or webview-wrapper agents** — most importantly Claude Desktop, where conversations are stored server-side by Anthropic and the local disk surface is just a Chromium webview cache. The same applies to ChatGPT desktop, future Anthropic-mobile, and any other MCP-aware agent without a readable on-disk store.

For these agents, use the write-side flow instead: the agent's model calls Bourdon's `commit_to_federation` MCP tool whenever it has context worth sharing. Bourdon writes that contribution to `~/agent-library/agents/<agent_id>.l5.yaml` atomically. No participant code, no scraping, no per-agent reverse-engineering.
...
- Read-side participants apply visibility filtering at query time regardless
...
This pattern is the canonical fit for **every future cloud-only or webview-based MCP-aware agent**. ChatGPT desktop, Anthropic mobile, browser-based agents, hosted agent frameworks — they all federate via the same `commit_to_federation` call. No per-agent participant code. No filesystem reverse-engineering. The agent's model picks what to share; Bourdon persists.

For agents that *do* have a readable local store, prefer the participant path documented above — automatic capture beats deliberate commit when you can get it.

## Paginating `list_recent_work` (issue #48)

`L6Store.list_recent_work` returns one **page** of sessions, capped at 20 by default and 100 by `MAX_LIMIT`. Participants or external clients that want the full history walk pages via the `cursor` token in the response:
...
If you ship a participant, please open a PR to `getbourdon/bourdon` with the participant module + tests + agent-integration-status update. We'll review against this guide and the spec doc. Participants that follow this guide tend to merge in one round; participants that skip the visibility-test category or roll their own redaction tend to need a second pass.
