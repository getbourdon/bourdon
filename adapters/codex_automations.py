"""Codex automation adapter -- publish background run memory as L5 evidence."""
...
def _read_automation_toml(path: Path) -> dict[str, Any]:
    try:
        ...
        logger.warning("CodexAutomationsAdapter: cannot parse %s: %s", path, exc)
        return {}
...
def _read_memory_text(path: Path | None) -> str:
    ...
    try:
        ...
        logger.warning("CodexAutomationsAdapter: cannot read %s: %s", path, exc)
        return ""
...
class CodexAutomationsAdapter:
    """External adapter for Codex automation memory artifacts."""
