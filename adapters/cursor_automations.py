"""Cursor automation adapter -- publish background agent run memory as L5 evidence.

Mirrors the Codex automation publisher. Reads the
``~/.cursor/automations/<id>/`` convention:
...
that the interactive-only ``adapters.cursor`` adapter leaves behind:
...
def _read_automation_toml(path: Path) -> dict[str, Any]:
    try:
        ...
        logger.warning("CursorAutomationsAdapter: cannot parse %s: %s", path, exc)
        return {}
...
def _read_memory_text(path: Path | None) -> str:
    ...
    try:
        ...
        logger.warning("CursorAutomationsAdapter: cannot read %s: %s", path, exc)
        return ""
...
class CursorAutomationsAdapter:
    ...
    def __init__(
        ...
        self._automations_dir = automations_dir or default_cursor_automations_dir(cursor_home)
    ...
    def health_check(self) -> HealthStatus:
        if not self._automations_dir.is_dir():
            return HealthStatus(
                status="blocked",
                reason=f"Cursor automations directory not found at {self._automations_dir}.",
