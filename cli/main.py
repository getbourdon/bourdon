...
def _handle_cursor_export(args: argparse.Namespace) -> int:
    """Build a Cursor L5 manifest. Hook-safe: silent on success, returns 0 in
    all failure modes so a session-end hook never crashes. Use --verbose for
    stderr diagnostics."""
    ...
    try:
        ...
        if verbose:
            print(f"bourdon cursor export: adapter init failed: {exc}", file=sys.stderr)
        return 0
    ...
    try:
        ...
        if verbose:
            print(
                f"bourdon cursor export: no Cursor data found ({exc}), skipping",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        ...
        if verbose:
            print(f"bourdon cursor export: write to {out_path} failed: {exc}", file=sys.stderr)
        return 0
...
def _handle_cursor_sync_native(args: argparse.Namespace) -> int:
    """Render federation content into a Cursor-readable markdown file."""
    from core.l6_store import DEFAULT_LIBRARY_PATH, L6Store
    ...
    all_sessions.sort(key=lambda pair: pair[1].get("date", ""), reverse=True)

    lines = [
        "# Bourdon Federation Context",
        "",
        f"_Auto-generated by `bourdon cursor sync-native`. {len(agents)} agents federated._",
        "",
    ]
    if all_entities:
    ...
    if all_sessions:
        ...
        for agent_id, session in all_sessions[:max_sessions]:
            ...
            action_text = "; ".join(str(a)[:120] for a in actions[:3]) if actions else ""
            line = f"- **{date}** ({agent_id})"
            if cwd:
                line += f" in `{cwd}`"
            if action_text:
                line += f": {action_text}"
    ...
    cursor_home = Path(args.cursor_dir) if getattr(args, "cursor_dir", None) else None
    target = (
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    cursor_compile_turn_cmd.add_argument("--library-path", help="Override agent-library root.")
    ...
    cursor_sync_mode.add_argument(
        ...
        help="Print what would be written (default).",
    )
    cursor_sync_mode.add_argument(
        "--write", action="store_true", default=False,
        help="Write ~/.cursor/memory/bourdon_context.md.",
    )
    cursor_sync_native_cmd.add_argument("--out", help="Override output path.")
